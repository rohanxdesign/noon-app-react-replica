...
export function FaqCard() {
  ...
  return (
    <SmoothCorners
      radius={12}
      className="bg-white flex flex-col gap-[12px] items-start pt-[16px] px-[12px] pb-[8px] relative w-[343px]"
    >
      <p className="font-semibold leading-[20px] text-[#171717] text-[16px] tracking-[-0.16px]">
      ...
      <div className="flex flex-col items-start w-full">
        {items.map(({ q, a }, i) => {
          ...
          return (
            <div
              key={i}
              className={`${isLast ? "" : "border-b border-[#f5f5f5]"} flex flex-col w-full`}
            >
              <button
                type="button"
                onClick={() => setOpenIndex(isOpen ? null : i)}
                aria-expanded={isOpen}
                className="flex gap-[16px] items-center py-[14px] w-full cursor-pointer text-left"
              >
                ...
                <img
                  alt=""
                  src={chevronDown1}
                  className="size-[20px] shrink-0 transition-transform duration-200"
              ...
              >
                <div className="overflow-hidden">
                  <p className="font-noontree text-[13px] leading-[19px] tracking-[-0.12px] text-[#666d85] pb-[14px] pr-[28px]">
                    {a}
                  </p>
                </div>
              </div>
